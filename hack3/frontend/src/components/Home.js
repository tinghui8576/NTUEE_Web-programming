    ...
    () => {
      subscribeToMore({
        ...
        updateQuery: (prev, { subscriptionData }) => {
          ...
          //console.log(id, 'id')
          // console.log(prev.items.findIndex(item => item.id === id))
          const Items = prev.items.filter(item => item.id !== id)
          //const Items = prev.items.splice(prev.items.findIndex(item => item.id === id), 1)
          console.log(Items)
          console.log(prev.items)
          console.log(subscriptionData)
          //console.log(prev.items.map((item) => (item.id === id ? delete : item)))
          return {
            items: [...Items],
          };
        },
